<template>
  ...
  <nav role="navigation" class="w-full m-2 p-2 flex flex-row font-serious">
    ...
    <t-dropdown  class="w-full" >
      ...
      <div slot-scope="{ hide, blurHandler }" class="origin-top-right absolute right-0 w-56 shadow-lg text-md bg-gray-200">
        ...
        :key="row.title"
        class="block w-full px-4 py-2 leading-5  text-gray-900 hover:bg-gray-600 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
        role="menuitem"
        @blur="blurHandler"
        >
          {{ row.title }}
